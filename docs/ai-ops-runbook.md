...
This runbook is for operators running AI over records in a sovereign or
air-gapped deployment, and for anyone responsible for routine AI audits. It
...
It applies to **both** deployments - the AtoM-AHG plugins and Heratio (the
Laravel platform). Where a command differs the runbook gives both; AtoM-AHG
uses `php symfony`, Heratio uses `php artisan`.

## Deployment modes

The AtoM-AHG plugins and Heratio are AI *clients* - they never host a model.
...
- [ ] HTR is resolved from the `app_htr_url` config (AtoM-AHG) / `htr_url`
      setting (Heratio) - confirm it is the local gateway.
...
4. The private key is written to a gitignored directory -
   `data/ahg-ai-signing/` (AtoM-AHG) or `storage/app/ai-signing/` (Heratio) -
...
| Action | AtoM-AHG (Symfony) | Heratio (Laravel) |
|---|---|---|
| Generate signing keypair | `php symfony ai-provenance:keygen` | `php artisan ahg:provenance-ai:keygen` |
| Rotate signing keypair | `php symfony ai-provenance:keygen --force` | `php artisan ahg:provenance-ai:keygen --force` |
| Verify signatures | `php symfony ai-provenance:verify` | via the Governance dashboard / `InferenceSigner` |
| Replay deferred Fuseki writes | `php symfony ai-provenance:replay` | `php artisan ahg:provenance-ai:replay` |
| Governance dashboard | `/ai/governance` | `/admin/governance` |

Run symfony / artisan tasks as the web user (`sudo -u www-data ...`) so they do
