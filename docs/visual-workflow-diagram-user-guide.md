...
When a task is approved, the workflow service checks the Collections Procedures chain rules (if any are configured) and may automatically spawn a downstream task — see the *Collections Procedures compliance dashboard* article for how chain rules work.
